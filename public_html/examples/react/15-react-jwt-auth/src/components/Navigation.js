...
export function Navigation(props) {
    ...
    if (user) {
        ...
        navigationItems.push({title: `Welcome, ${user.username}!`, link: "/profile"});
        navigationItems.push({title: "Logout", link: "", action: props.logoutFunction});
    } else {
    ...
    for (let i = 0; i < navigationItems.length; ++i) {
        ...
        let anchor;
        if (navItem.action) {
            anchor = <button className="link" onClick={navItem.action}>{navItem.title}</button>;
        } else {
